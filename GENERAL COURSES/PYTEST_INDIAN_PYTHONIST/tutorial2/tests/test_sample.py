from unittest import mock
from unittest.mock import call

from tutorial2.myapp.sample import random_sum, silly


@mock.patch("tutorial2.myapp.sample.random.randint")  # here's the path of the function you want to mock
def test_random_sum(mock_randint):  # here's the mock object of the function
    mock_randint.side_effect = [3, 4]  # these are the values you want to set for the function to receive it
    assert random_sum() == 7

    mock_randint.assert_has_calls(calls=[call(1, 10), call(1, 10)])


@mock.patch("tutorial2.myapp.sample.random.randint")
@mock.patch("tutorial2.myapp.sample.time.time")
@mock.patch("tutorial2.myapp.sample.requests.get")
def test_silly(mock_randint, mock_time, mock_requests):
    test_params = {
        "timestamp": 123,
        "number": 5
    }

    mock_time.return_value = test_params['timestamp']
    mock_randint.return_value = 5
    mock_requests.return_value = mock.Mock(**{"status_code": 200,
                                              "json.return_value": {"args": test_params}})
    assert silly() == test_params
